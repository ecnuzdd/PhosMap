...
get_global_background_df <- function(species = 'human', fasta_type = 'refseq'){
  ...
  if(!file.exists(BACKGROUND_FILE_PATH)){
    PHOSPHATE_LIB_MOTIF_BG_FILE_ftp_link <- 'ftp://111.198.139.72:4000/pub/PhosMap_datasets/motif_library/fasta_type/species/STY_background_of_fasta_type_species_for_motif_enrichment.txt'
    PHOSPHATE_LIB_MOTIF_BG_FILE_ftp_link <- stringr::str_replace_all(PHOSPHATE_LIB_MOTIF_BG_FILE_ftp_link, 'fasta_type', fasta_type)
    PHOSPHATE_LIB_MOTIF_BG_FILE_ftp_link <- stringr::str_replace_all(PHOSPHATE_LIB_MOTIF_BG_FILE_ftp_link, 'species', species)

    PHOSPHATE_LIB_MOTIF_BG_FILE_data_type <- 'txt'

    PHOSPHATE_LIB_MOTIF_BG_DATA <- tryCatch(
      {
        load_data_with_ftp(PHOSPHATE_LIB_MOTIF_BG_FILE_ftp_link, PHOSPHATE_LIB_MOTIF_BG_FILE_data_type)
      },
      error=function(cond) {
    ...
    if(!is.list((PHOSPHATE_LIB_MOTIF_BG_DATA))){
      ...
      cat('\n')
      message('The another option is that you can directly download the background data from ', PHOSPHATE_LIB_MOTIF_BG_FILE_ftp_link, ' and then put it into ', BACKGROUND_FILE_PATH)
      stop()
    }
